    ...
    stages{
        ...
        stage('upload artifacts to jfrog'){
            steps{
                 ...
                 *ansible-${BUILD_ID}.zip "http://54.152.26.224:8081/artifactory/ansible-repo/ansible-${BUILD_ID}.zip"'
            }
        }
        stage('publish over ssh'){
            steps{
              sshPublisher(publishers: [sshPublisherDesc(configName: 'AnsibleServer', transfers: [sshTransfer(cleanRemote: false, \
               excludes: '', execCommand: 'unzip -o ansible-${BUILD_ID}.zip; rm -rf ansible-${BUILD_ID}.zip ', execTimeout: 120000, flatten: false, makeEmptyDirs: false, \
